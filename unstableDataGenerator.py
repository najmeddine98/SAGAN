...
def build_discriminator(img_shape):
    model = Sequential()
    model.add(
    Conv2D(32,kernel_size=3,strides=(1,1),input_shape=img_shape,padding='same'))        #(5,4,,32)
    model.add(LeakyReLU(alpha=0.01))
    model.add(
    Conv2D(64,kernel_size=3,strides=1,input_shape=img_shape,padding='same'))    #(5,4,64)
    model.add(BatchNormalization())
    model.add(LeakyReLU(alpha=0.01))
    model.add(
    Conv2D(128,kernel_size=3,strides=2,input_shape=img_shape,padding='same'))    #(2,2,128)
    model.add(BatchNormalization())
    model.add(LeakyReLU(alpha=0.01))
    model.add(Flatten())
...
def sample_images(generator,iteration,image_grid_rows=4, image_grid_columns=4):
    ...
    for k in np.arange(40):
        i=Nbimg+k
        fileName = "Generated_Profile_temporelle_Unstable/%d.npy" % (i)
    ...
    for i in range(image_grid_rows):
        for j in range(image_grid_columns):
            ...
            axs[i, j].axis('off')
            cnt += 1
    fig.savefig("Generated_Profile_temporelle_Unstable_Img/%d.png" % iteration)
    plt.close()
...
#A=[]
#for k in np.arange(4559):

    #fileName = "Generated_Profile_temporelle_Unstable/%d.npy" % (k)
...
#print(A)
#print(np.shape(A))
#for k in np.arange(500000,800000):
 ...
 #   i=k-490737
  #  image_nb= "Generate_img/%d.jpg" % (i)
   ...
   # with open(fileName, 'rb') as f:
    ...
    #if (k % 500 == 0):
     #   print(k)
#python main.py --dataset ../Profile_temporelle/ProfileTemporelleUnStable
